    ...
    <Pressable style={{ backgroundColor: scale(0).hex() }} {...rest}>
      <View style={{ flexDirection: "row", alignItems: "center", marginVertical: 12 }}>
        ...
        <View style={{ flex: 1, marginHorizontal: 16 }}>
          {overline &&
            <Text selectable={false} variant="overline"
                  style={{ marginBottom: 2, color: scale(0.6).hex() }}>{overline}</Text>}
          ...
          {secondaryText &&
            <Text selectable={false} variant="body2"
                  style={{ marginTop: 4, color: scale(0.6).hex() }}>{secondaryText}</Text>}
        </View>
        {trailing &&
          <View style={{
            width: 24,
