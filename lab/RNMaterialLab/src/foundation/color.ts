...
import { Elevation, ThemeMode } from "./";
...
export type Colors = Record<Color, ColorVariants> & { surfaceVariants: Record<Elevation, string> }
...
const on = (color: string) => chroma.contrast(color, "white") > 4.5 ? "white" : "black";

export const createColors = (mode: ThemeMode = "light", options: ColorsOptions = {}): Colors => {
  ...
  const scale = (value: number) => mode === "light" ? surface : f(value).hex();
